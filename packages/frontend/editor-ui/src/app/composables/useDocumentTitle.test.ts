...
describe('useDocumentTitle', () => {
	it('should set the document title', () => {
		...
		set('Test Title');
		expect(document.title).toBe('Test Title - BOTO Workflow');
	});
	...
	it('should reset the document title', () => {
		...
		set('Test Title');
		reset();
		expect(document.title).toBe('Workflow & Agent Orchestration - BOTO Workflow');
	});
	...
	it('should use the correct prefix for the release channel', () => {
		...
		const { set } = useDocumentTitle();
		set('Test Title');
		expect(document.title).toBe('Test Title - BOTO Workflow[BETA]');
	});
	...
	describe('setDocumentTitle', () => {
		...
		it('should set document title with IDLE status', () => {
			...
			expect(document.title).toBe('▶️ My Workflow - BOTO Workflow');
		});

		it('should set document title with EXECUTING status', () => {
			const { setDocumentTitle } = useDocumentTitle();
			setDocumentTitle('My Workflow', 'EXECUTING');
			expect(document.title).toBe('🔄 My Workflow - BOTO Workflow');
		});
		...
		it('should set document title with AI_BUILDING status', () => {
			...
			expect(document.title).toBe('[Building] My Workflow - BOTO Workflow');
		});

		it('should set document title with AI_DONE status', () => {
			const { setDocumentTitle } = useDocumentTitle();
			setDocumentTitle('My Workflow', 'AI_DONE');
			expect(document.title).toBe('[Done] My Workflow - BOTO Workflow');
		});
	});
	...
	describe('claimDocumentTitle', () => {
		...
		it('should ignore workflow title updates while a host owns the title', () => {
			...
			expect(document.title).toBe('My conversation - BOTO Workflow');
			scope.stop();
		});
